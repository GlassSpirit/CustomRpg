...
class EntCorpse(worldIn: World) : Entity(worldIn), IInventory {

    private var drops: MutableList<ItemStack> = ArrayList()
    private var owner: UUID? = null
    var oldEntityData: NBTTagCompound? = null
        private set
    ...
    private fun copyData(entityLivingBase: EntityLivingBase) {
        ...
        oldEntityData!!.setShort("HurtTime", 0.toShort())
    }
    ...
    override fun onEntityUpdate() {
        ...
        if ((entityClass.isEmpty()) && world.isRemote) {
            RPGLoot.packetHandler.sendToServer(ReqCorpseSyncPacket(this))
        }

        if (world.isRemote && RPGLoot.config.looting && rand.nextBoolean() && drops.size > 0) {
    ...
    override fun markDirty() {
        ...
        RPGLoot.packetHandler.sendToAllAround(CorpseSyncPacket(this),
                TargetPoint(dimension, posX, posY, posZ, 64.0))
    }
