...
public class Bank {
    ...
    public boolean canBeUpgraded(int slot) {
        if (upgradeInventory.getStackInSlot(slot) == null || upgradeInventory.getStackInSlot(slot).isEmpty())
            return false;
    ...
    public int getMaxSlots() {
        for (int i = 0; i < maxSlots; i++) {
            if ((currencyInventory.getStackInSlot(i) == null || currencyInventory.getStackInSlot(i).isEmpty()) && i > startSlots - 1)
                return i;
        }
        return maxSlots;
    }
