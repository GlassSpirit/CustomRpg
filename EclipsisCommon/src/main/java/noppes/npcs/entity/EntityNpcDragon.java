    ...
    public void onLivingUpdate() {
        ...
        if (world.isRemote && getHealth() <= 0) {
            if (!exploded) {
                ...
                world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, posX + (double) f, posY + 2D + (double) f2, posZ + (double) f4, 0.0D, 0.0D, 0.0D);
            }
        } else {
            exploded = false;
            //func_41011_ay();
            float f1 = 0.2F / (MathHelper.sqrt(motionX * motionX + motionZ * motionZ) * 10F + 1.0F);
            f1 = 0.045f;
