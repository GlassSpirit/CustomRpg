...
public class EntityAIOpenAnyDoor extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (pathentity != null && !pathentity.isFinished()) {
            for (int i = 0; i < Math.min(pathentity.getCurrentPathIndex() + 2, pathentity.getCurrentPathLength()); ++i) {
                ...
                position = new BlockPos(pathpoint.x, pathpoint.y + 1, pathpoint.z);
    ...
    public void setDoorState(Block block, BlockPos position, boolean open) {
        if (block instanceof BlockDoor) {
            ((BlockDoor) block).toggleDoor(npc.world, position, open);
        } else {
            IBlockState state = npc.world.getBlockState(position);
            if (state.getBlock() != block)
                return;
            npc.world.setBlockState(position, state.withProperty(property, open));
            npc.world.playEvent(null, open ? 1003 : 1006, position, 0);
        }
    }
}
