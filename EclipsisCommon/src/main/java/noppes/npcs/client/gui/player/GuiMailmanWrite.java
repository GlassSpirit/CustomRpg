...
public class GuiMailmanWrite extends GuiContainerNPCInterface implements ITextfieldListener, IGuiError, IGuiClose, GuiYesNoCallback {
    ...
    public void initGui() {
        ...
        if (canEdit && !canSend)
            addTextField(new GuiNpcTextField(2, this, fontRenderer, guiLeft + 170, guiTop + 42, 114, 20, mail.sender));
        else if (canEdit)
            addTextField(new GuiNpcTextField(0, this, fontRenderer, guiLeft + 170, guiTop + 42, 114, 20, username));
        else
        ...
        if (canEdit)
            addTextField(new GuiNpcTextField(1, this, fontRenderer, guiLeft + 170, guiTop + 82, 114, 20, mail.subject));
        else
    ...
    private void func_74160_b(String par1Str) {
        ...
        int i = mc.fontRenderer.getWordWrappedHeight(s2 + "" + TextFormatting.BLACK + "_", 118);
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        s = net.minecraft.client.resources.I18n.format("book.pageIndicator", Integer.valueOf(this.currPage + 1), Integer.valueOf(this.bookTotalPages));
        s1 = "";
    ...
    public void setError(int i, NBTTagCompound data) {
        if (i == 0)
            error.label = I18n.translateToLocal("mailbox.errorUsername");
        if (i == 1)
            error.label = I18n.translateToLocal("mailbox.errorSubject");
        hasSend = false;
    }

    @Override
    public void setClose(int i, NBTTagCompound data) {
        player.sendMessage(new TextComponentTranslation("mailbox.succes", data.getString("username")));
    }
