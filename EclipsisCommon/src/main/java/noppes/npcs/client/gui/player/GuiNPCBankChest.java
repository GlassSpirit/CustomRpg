...
public class GuiNPCBankChest extends GuiContainerNPCInterface implements IGuiData {
    ...
    public void initGui() {
        ...
        if (maxSlots > 1) {
            for (int i = 0; i < maxSlots; i++) {
                GuiNpcButton button = new GuiNpcButton(i, guiLeft - 50, guiTop + 10 + i * 24, 50, 20, I18n.translateToLocal("gui.tab") + " " + (i + 1));
                if (i > unlockedSlots)
        ...
        if (!container.isAvailable()) {
            addButton(new GuiNpcButton(8, guiLeft + 48, guiTop + 48, 80, 20, I18n.translateToLocal("bank.unlock")));
        } else if (container.canBeUpgraded()) {
            addButton(new GuiNpcButton(9, guiLeft + 48, guiTop + 48, 80, 20, I18n.translateToLocal("bank.upgrade")));
        }
        if (maxSlots > 1) {
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        if (!container.isAvailable()) {
            ...
            int x = guiLeft + 30;
            int y = guiTop + 8;
            fontRenderer.drawString(I18n.translateToLocal("bank.unlockCosts") + ":", x, y + 4, CustomNpcResourceListener.DefaultTextColor);
            ...
            int x = guiLeft + 30;
            int y = guiTop + 8;
            fontRenderer.drawString(I18n.translateToLocal("bank.upgradeCosts") + ":", x, y + 4, CustomNpcResourceListener.DefaultTextColor);
            drawItem(x + 90, y, currency, i, j);
        } else {
    ...
    private void drawItem(int x, int y, ItemStack item, int mouseX, int mouseY) {
        ...
        itemRender.renderItemOverlays(fontRenderer, item, x, y);
    ...
    public void setGuiData(NBTTagCompound compound) {
        ...
        if (compound.hasKey("Currency"))
            currency = new ItemStack(compound.getCompoundTag("Currency"));
        else
