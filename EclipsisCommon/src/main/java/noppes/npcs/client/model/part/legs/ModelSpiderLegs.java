...
public class ModelSpiderLegs extends ModelRenderer {
    ...
    public void setRotationAngles(ModelData data, float par1, float par2, float par3, float par4, float par5, float par6, Entity entity) {
        ...
        if (((EntityNPCInterface) entity).isPlayerSleeping() || ((EntityNPCInterface) entity).currentAnimation == AnimationType.CRAWL) {
            rotationPointY = 12 * data.getPartConfig(EnumParts.LEG_LEFT).scaleY;
            rotationPointZ = 15 * data.getPartConfig(EnumParts.LEG_LEFT).scaleY;
